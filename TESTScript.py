...
def load_model(path):
    return Generator.load_from_checkpoint(path)
...
def autoregression(model, path, model_name, columns, X, edge_index, edge_weight, enc_dec_mean, add_noise, args, kwargs, adj_name=None):
        model_output = model.autoregression(X=X, edge_index=edge_index, edge_weight=edge_weight, steps=1000, add_noise=add_noise, end_dec_mean=enc_dec_mean, **kwargs)
        model_output = pd.DataFrame(model_output[0, :, :, 0])
        model_output.columns = columns
        save_data(model_output, path, model_name + ('_enc_dec_mean' if enc_dec_mean else '') + ('_noise' if add_noise else '') + '_w' + str(args.seq_length) + (f'_{adj_name}' if adj_name is not None else ''))   
...
def run(args):
    ...
    for model in models:
        ...
        if args.test_different_adj:
            edge_index_self = torch.tensor(adj_self[0])
            edge_weight_self = torch.tensor(adj_self[1])
            edge_index_sparse = torch.tensor(adj_sparse[0])
            edge_weight_sparse = torch.tensor(adj_sparse[1])
            
        if args.mode.lower() =='g':
            X = y_true[-20:].permute(1, 0, 2, 3)
            
            for enc_dec_mean in [True, False]:
                for noise in [True, False]:
                    autoregression(ckpt, resulting_path, modelresulting_name, columns, X, edge_index, edge_weight, enc_dec_mean, noise, args, kwargs)
            
            if args.test_different_adj:
                for enc_dec_mean in [True, False]:
                    for noise in [True, False]:
                        autoregression(ckpt, resulting_path, modelresulting_name, columns, X, edge_index_self, edge_weight_self, enc_dec_mean, noise, args, kwargs, 'self')
                for enc_dec_mean in [True, False]:
                    for noise in [True, False]:
                        autoregression(ckpt, resulting_path, modelresulting_name, columns, X, edge_index_sparse, edge_weight_sparse, enc_dec_mean, noise, args, kwargs, 'sparse')
            
            save_model_data(models[model], resulting_path, modelresulting_name + '_PARAMS_w' + str(args.seq_length))
        elif args.mode.lower() == 'i':
...
if __name__ == '__main__':
    ...
    parser.add_argument('-mode', type=str, default='g', choices=['p', 'i', 'g'])
