...
class GTR(nn.Module):
    def __init__(self, input_size, output_size, hidden_size, weight_decay, window, horizon, dropout, num_layers, bidirectional, loss, lr, callbacks, device, exo_size=0) -> None:
        ...
        self.rnn = nn.RNN(input_size=input_size+exo_size, hidden_size=hidden_size, dropout=dropout, num_layers=num_layers, device=device, bidirectional=bidirectional, batch_first=True)
