...
def reshape_to_original(final_tensor: torch.Tensor, num_nodes: int, input_shape: int, mixture_size: int) -> torch.Tensor:
    ...
    weights_size = mixture_size # Size of stds
    ...
    stds = stds.reshape(-1, stds.shape[-1] * stds.shape[-2])[..., :stds_size]
    weights = weights.reshape(-1, weights.shape[-1] * weights.shape[-2])[..., :weights_size]

    original_tensor = torch.cat([means_flat, stds, weights], dim=-1)
