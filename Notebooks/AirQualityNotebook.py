...
from Notebooks.MMD import mmd_rbf, mmd_linear, mmd_poly
...
filename = '/data/p.magos/TSGen/logs/generation/AirQuality/WAIRSmall-model-epoch=07-val_loss=1.6367.ckpt'
...
generation = generator.generate(input, torch.tensor(adj[0]), torch.Tensor(adj[1]), None, 2000, both_mean=True, **kwargs)
...
plt.plot(prediction[-100:-1, 1], label='Predicted')
...
# %%
err = torch.square(prediction[-99:-1] - true[-98:])
err.mean()
